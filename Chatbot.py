...
data = pd.read_csv("C:/CHatbot 1503/Emotionally-Intelligent-Chatbot-for-Mental-Healthcare-and-suicide-prevention-master/labeledTrainData.tsv", header=0, \
                    delimiter="\t", quoting=2)
...
def depression(name):
    ...
    time.sleep(1)
    print(bot.format('I am sorry to hear that you are feeling as if you are in a slump. First of all, nothing is wrong with you. Many people report similar symptoms. Have you ever talked with a physician regarding your symptoms? Two extreme emotions such as feeling extremely happy and then feeling extremely low for a length of time may indicate a mood disorder.'))
    time.sleep(1)
    print(bot.format('Here is a thought that might motivate you!'))
    time.sleep(1)
    print(bot.format('There you go...let it all slide out.Unhappiness cannot stick in a person\'s soul when it\'s slick with tear. and there is hope even your brain convinces you there isn\'t '))
...
def anxiety(name):
    ...
    print(bot.format(''))
    time.sleep(1)
    print(bot.format('Consider yourself quite normal for feeling overwhelmed considering the times that we are living in.'))
    time.sleep(1)
    print(bot.format('Take a deep breath, listen to your thoughts, try to figure them out. Then take things one day at a time. One small crack does not mean that you are broken, it means that you were put to the test and you didn’t fall apart'))
...
def paranoia(name):
    ...
    time.sleep(1)
    print(bot.format('It\'s not unusual for traumatic experiences that happened when we were younger to stay with us when we get older. Traumatic experiences can become embedded in our bodies, as well as in our emotions often leading to us feeling paranoid'))
    time.sleep(1)
...
def personality_disorder(name):
    ...
    print(bot.format('Here is a thought that might motivate you!'))
    time.sleep(1)
    print(bot.format('...repeated trauma in childhood forms and deforms the personality. The child trapped in an abusive environment is faced with formidable tasks of adaptation. She must find a way to preserve a sense of trust in people who are untrustworthy, safety in a situation that is unsafe, control in a situation that is terrifyingly unpredictable, power in a situation of helplessness. You are not your illness. You have an individual story to tell. You have a name, a history, a personality. Staying yourself is part of the battle'))
...
def substance_abuse(name):
    ...
    print(bot.format('Here is a thought that might motivate you!'))
    time.sleep(1)
    print(bot.format(' My Recovery Must Come First So That Everything I Love In Life Doesn’t Have To Come Last. Positive vibes only’ isn’t a thing. Humans have a wide range of emotions and that’s OK'))
...
def greet():
    ...
    while True:
        ...
        if(negative == 0):
            ...
            time.sleep(1)
            print(bot.format("In general, when a small thing bothers us (and this happens to all of us), it's because the small thing triggers an emotion in us that we have felt 'too much' or 'too intensely' in the past, and we dont know how to manage that emotion effectively...we just want to avoid it as quickly as possible.,I have got great tools for people dealing with stress,wanna give it a go,Yes/No?"))
            time.sleep(1)
            message = input().lower()
            if  message == 'yes':
